	  ...
	  messageTextBox (0)
{
    ...
    addAndMakeVisible (testliste = new ListBox(T("testliste")));
	...
	setSize (800, 600);
    TextButton* t;
    testliste->addAndMakeVisible(t= new TextButton(T("t")));
    t->setBounds(0,0,100,25);
...
void Window::resized()
{
    ...
    testliste->setBounds(500,400,100,100);
...
bool Window::runFunction(int function, const char* text){
	...
	switch(function){
		...
		case CONNECT:
			...
			if(success){
				addToBox("Connected with Server\n",statusTextBox);
				addToBox("Send a Tweetername to Login\n",statusTextBox);
				pthread_t pid;
		...
		case SEND:
			...
			if(success)addToBox("Message sended\n",statusTextBox);
			break;
	}
	if(!success){
	...
	//if(success){
		if(message.startsWithChar('m')){
			//message.substring(1);
			 ...
			 addToBox(t.toCString(),messageTextBox); 
		}

		addToBox(message.toCString(),messageTextBox);
	}
