...
void Window::buttonClicked (Button* buttonThatWasClicked)
{
    if (buttonThatWasClicked == sendingButton)
    {
        if(!sendingTextBox->isEmpty()){
            runFunction(SEND, sendingTextBox->getText().toCString());
...
void Window::handleMessage(){
    bool success;
    String message = tweety.receive(&success); 
    if(message.isNotEmpty()){
    //if(success){
        if(message.startsWithChar('m')){
            int firstCat = message.indexOfChar(';');
            int secondCat = message.indexOfChar(firstCat+1,';');
            addToBox("*********************************************",messageTextBox);
            addToBox(message.substring(secondCat+1).toCString(),messageTextBox);
            
            addToBox("--------------",messageTextBox);
            String time = message.substring(1,firstCat).toCString();
            convertTimestamp(&time);
            time = "At: " + time;
            addToBox(time.toCString(), messageTextBox);
            
            String sender = message.substring(firstCat+1,secondCat);
            sender = "From: " + sender;
            addToBox(sender.toCString(), messageTextBox);
            
            addToBox("--------------",messageTextBox);
        }
        else
        {
            addToBox(message.toCString(),statusTextBox);
        }
    }
}

void Window::convertTimestamp(String* time){
    String s("");
    s = time->substring(8,10) + String(":") + time->substring(10,12) + String(":") + time->substring(12,14);
    s += String(" ") +  time->substring(6,8) + String(".") + time->substring(4,6) + String(".") + time->substring(0,4);
    *time = s;
}
